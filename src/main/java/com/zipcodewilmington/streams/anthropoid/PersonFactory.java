...
public final class PersonFactory {
    ...
    public List<Person> createPersonList(int listSize) {
        ...
        Arrays.stream(new int[listSize]).forEach((x) -> people.add(createRandomPerson()));
        return people;
    }
    ...
    public Person[] createPersonArray(int arrayLength) {
        ...
        Integer[] range = IntStream.rangeClosed(0, arrayLength-1)
                .boxed().collect(Collectors.toList()).toArray(new Integer[arrayLength]);
        Arrays.stream(range).forEach((x) -> people[x] = createRandomPerson());
        return people;
    }
